function AIScheduleTools () {
  ...
  const AIScheduleSelect = ({
    ...
    const errorText = createErrorText()
    const select = createSelect(selectList, () => {
      errorText.innerText = ''
    })
    const btn = createBtn()
  ...
  const AIScheduleLoading = ({
    ...
    const content = createContent(contentText)
    ...
    const dynLoading = () => {
      const t = ['', '.', '..', '...']
      if (count === 4) count = 0
      content.innerText = `${contentText}${t[count++]}`
    }
    return {
      show () {
        document.body.appendChild(mask)
        dyn = setInterval(dynLoading, 500)
      },
      close () {
  ...
  return { AIScheduleAlert, AISchedulePrompt, AIScheduleConfirm, AIScheduleSelect, AIScheduleLoading }
}
